...
Using the example in~\S\ref{sub:Comp-of-concepts}, a C-element with environment,
...
As mentioned in~\S\ref{sub:scenarios}, concepts are used to specify
...
\vspace{1mm}
\begin{center}
\includegraphics[scale=0.4]{Images/sequential_combination}
\end{center}
...
\begin{center}
\includegraphics[scale=0.4]{Images/concurrent_combination}
\end{center}
...
\vspace{1mm}
\begin{center}
\includegraphics[scale=0.4]{Images/non_deterministic_combination}
\end{center}
...
In~\S\ref{sec:Motivating Example} we introduced the motivating example for
...
concepts in~\S\ref{sec:Concepts}, and with examples explained how they can
be used to design simple circuits, and how these can be translated to
provide us with an STG specification for use with standard tools. 
Now we detail a design flow supported by \noun{Workcraft} for producing formal STG
specifications for more complex circuits and use the buck converter as a case study.
...
Informal buck specification defines three operating modes of the analogue circuit
that require distinctive control scenarios. Requirements for each operating mode
can be captured with a separate list of concepts, which are translated into scenario
STGs. These can be subsequently combined to produce a single full circuit STG.

During this process, it is useful to find any operations which occur
between two or more operational modes, as these can then be reused -- a concept defined
in one list of concepts can be referenced from other scenarios by name.
...
\vspace{-4mm}
\[
\mathsf{circuit}(uv,oc,zc,gp,gp\_ack,gn,gn\_ack)~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
\]
\vspace{-6mm}
\[
~~=~\mathsf{chargeFunc}~\diamond~\mathsf{uvFunc}~\diamond~\mathsf{uvReact}~\diamond~\mathsf{zcAbsent}
\]
\vspace{-5mm}
\[
\begin{array}{lcl}
...
~&\hspace{-2mm}\diamond&\hspace{-2mm}\mathsf{environment}~\diamond~\mathsf{noShortcircuit}~\diamond~\\
...
We also include an \textsf{interface} concept in order to specify the
